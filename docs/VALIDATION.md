...
- Advanced AI word cards, Reader long-press sentence translation/analysis mode
  switching, and visible-page paragraph translation in paginated and continuous
  modes. Inline translations must stay below their source text and must not enter
  lookup selection or progress counts; VN must continue to report the feature as
  unsupported.
...
- recursive Dictionary folder import with standard Yomitan ZIP files, imported
  native dictionary directories, unrelated files, and invalid directories;
  valid dictionaries must retain their detected title and data types.
