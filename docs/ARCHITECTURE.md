...
- `features/advancedai` owns Advanced AI settings persistence, OpenAI-compatible
  word and sentence requests, popup AI render state, and the shared content
  plumbing used by Reader lookup, Process Text lookup, and Anki field rendering.
...
- Dictionary import and imported-directory inspection, term/Kanji lookup, media,
  style extraction, deinflection, frequency, and complete pitch data are owned by
...
- Reader sentence translation/analysis cards and full-page visible-paragraph
  ...
  paginated and continuous modes without letting translated text feed back into
  Reader progress or selection semantics.
